...
function RNN.createModel(input_size,hidden_size,rnn_size,heads)
	...
	for i=1,heads do
		output[i]=hidden[rnn_size] - nn.Linear(hidden_size,input_size) --outputs
	end
	--ANNOTATIONS
...
function RNN.trainUnfoldedModel(rnn,learning_rate,data,head_no,batch_size)
	...
	while BatchManager.isNextBatch(batcher) do
		...
		for r=1,rnn.rnn_size do
			hiddenGradient[rnn.seq_size][r]=ZERO_HIDDEN_VECTOR --passing only reference
		end
