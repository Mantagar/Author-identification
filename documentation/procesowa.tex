...
\begin{enumerate}
	...
	\item zaimplementowanie programu testuj±cego wydajno¶æ nauczonych sieci
	\item refaktoryzacja, modularyzacja kodu oraz testowanie
\end{enumerate}
...
Nawet sieci uczone krótko (nie wiêcej ni¿ 5 epok) wykazywa³y skuteczno¶æ weryfikacji autorstwa relatywnie wysok±. Podczas testowania wspó³czynnik weryfikacji nieznanych tekstów w jêzyku angielskim zawiera³ siê w przedziale 0.6-0.9 w zale¿no¶ci od ilo¶ci autorów i czasu treningu.
\subsection{Propozycje dalszego rozwoju}
Sieci definiowane przez program u¿ywaj± zwyk³ego algorytmu SGD, aby poprawiæ ich wydajno¶æ mo¿na by dodaæ pewne optymalizacje, np. za pomoc± pakietu optim.
Dziêki zastosowaniu batchów nauka przebiega o wiele szybciej, choæ z wykorzystaniem karty graficznej i pakietów cuda lub cunn mo¿liwe jest osi±gniêcie przyspieszenia wielokrotnie wiêkszego.
Mo¿na by próbowaæ pomy¶leæ nad wprowadzeniem lepszych metod weryfikacji autorstwa na podstawie wyników sieci.

\section{Dodatki}
\subsection{Spotkania z opiekunem}
\emph{14.03.2017}
\newline
Spotkanie w celu omówienia tematu pracy. Zosta³em zapoznany ze szczegó³ami oraz zosta³y mi przedstawione dokumenty bêd±ce podstaw± tej pracy, jak równie¿ zosta³em poinformowany potrzebie nauczenia siê jêzyka Lua oraz obs³ugi frameworka Torch.
\newline
\newline
\emph{28.03.2017}
\newline
...
\emph{23.05.2017}
\newline
Spotkanie prezentuj±ce postêpy w pracach nad projektem. Zosta³em poinformowany o mo¿liwo¶ci wykorzystania klastra Prometheus do testów sieci oraz zosta³em dodany do granta do dostêpu do superkomputera. Uzgodnili¶my równie¿ wtedy utworzenie repozytorium w systemie GitHub w celu nadzorowania dalszych postêpów prac. Projekt jest dostêpny pod adresem: \url{https://github.com/Mantagar/Author-identification}
...
\begin{enumerate}
	...
	\item Na trzecim prezentowali¶my analizê ryzyka oraz studium wykonalno¶ci naszych projektów oraz wymagania funkcjonalne i niefunkcjonalne.
	\item Na czwartym i ostanim spotkaniu prezentowali¶my postêpy prac nad projektami.
\end{enumerate}
